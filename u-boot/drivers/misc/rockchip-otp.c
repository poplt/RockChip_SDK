...
static int rockchip_otp_probe(struct udevice *dev)
{
    ...
    printf("%s: probe called for device %s\n", __func__, dev->name);

    data = (struct otp_data *)dev_get_driver_data(dev);
    if (!data) {
        printf("%s: no driver data!\n", __func__);
        return -EINVAL;
    }

    if (data->init) {
        printf("%s: init function exists, calling it\n", __func__);
        int ret = data->init(dev);
        printf("%s: init returned %d\n", __func__, ret);
        return ret;
    }

    printf("%s: no init function, probe done\n", __func__);
    return 0;
}
