...
static inline void __attribute((unused)) read_xs9922_audio_reg(struct xs9922 *xs9922) {
	u32 tmpval = 0;
	int i = 0;
    printk("xs9922 audio: ");
    for (;xs9922->cur_mode->audio_reg_list[i].addr != REG_NULL; i++) {
        xs9922_read_reg(xs9922->client, xs9922->cur_mode->audio_reg_list[i].addr, XS9922_REG_VALUE_08BIT, &tmpval);
        printk("addr:0x%04x val:0x%02x ", xs9922->cur_mode->audio_reg_list[i].addr, tmpval);
        if ((i + 1) % 5 == 0) printk("\n xs9922 audio: ");
    }
    printk("\n");
}
...
void switch_mode(struct xs9922 *xs9922)
{
	...
	ret |= xs9922_write_array(xs9922->client, xs9922->cur_mode->audio_reg_list);
    if (ret)
    {
...
//    printk("__xs9922_init in\n");
    ...
    //audio config
    xs9922_write_array(xs9922->client, xs9922->cur_mode->audio_reg_list);
    ...
    if(IS_ERR(fp)){
        ...
        ssize_t bytes_read = kernel_read(fp,board_config_buf,sizeof(board_config_buf),&pos);
        if (bytes_read < 0) {
 		   printk(KERN_ERR "open "config_file" fail!!!\n");
		}
		filp_close(fp,NULL);
...
static int __xs9922_start_stream(struct xs9922 *xs9922)
{
	...
	xs9922_write_array(client, xs9922_audio_codec);
			 ...
			 const struct i2c_device_id *id)
{
	...
	xs9922->cur_mode = &supported_modes[2];
    ...
    if (1)
	{
    	...
    	if (IS_ERR(read_thread)) {
			dev_err(dev, "kthread_create xs9922_readconfig failed\n");
    	} else {
        	wake_up_process(read_thread);
			dev_err(dev, "wake_up_process ok\n");
    	}
	}
